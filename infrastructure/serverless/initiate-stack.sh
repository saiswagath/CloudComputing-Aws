...
echo "Enter the SSLCertificate CRN: "
...
stackId=$(aws cloudformation create-stack --stack-name $stack_name --template-body \
 file://serverless.json --profile $account --region $vpcRegion  --capabilities CAPABILITY_NAMED_IAM --parameters \
ParameterKey=vpcName,ParameterValue=$vpcName \
ParameterKey=ImageID,ParameterValue=$ImageID \
ParameterKey=vpcCidr,ParameterValue=$vpcCidr \
ParameterKey=KeyPair,ParameterValue=$KeyPair \
ParameterKey=hostedZoneID,ParameterValue=$hostedZoneID \
ParameterKey=SSLCertificate,ParameterValue=$SSLCertificate \
